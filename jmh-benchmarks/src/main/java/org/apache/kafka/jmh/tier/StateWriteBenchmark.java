...
public class StateWriteBenchmark {
    ...
    private void writeState(TierPartitionState state) throws IOException {
        ...
        for (int i = 0; i < COUNT; i++) {
            state.append(new TierObjectMetadata(TOPIC, PARTITION, EPOCH,
    ...
    public void appendReadByteBufferBench() throws Exception {
        ...
        FileTierPartitionStateFactory factory = new FileTierPartitionStateFactory(BASE_DIR, SPARSITY);
        TierPartitionState state = factory.newTierPartition(new TopicPartition(TOPIC, PARTITION));
