...
class TierIndexTopicSerializationTest {
  ...
  def serializeDeserializeTest(): Unit = {
    roundTrip(new TierTopicInitLeader("my-topic33", 0, 0, UUID.randomUUID, 33))
    roundTrip(new TierTopicInitLeader("my", 199999, 1, UUID.randomUUID, 99))
    roundTrip(new TierObjectMetadata("foo", 0, 0, 0L, 33333, 0L, 99999L, 100000L, 3333, false, kafka.tier.serdes.State.AVAILABLE))
  }
  ...
  def metadataIllegalEpochTest(): Unit = {
    new TierObjectMetadata("foo", 0, -1, 0L, 23252334, 0L,
      0L, 98, 102, false, kafka.tier.serdes.State.AVAILABLE)
  }

  @Test (expected = classOf[IllegalArgumentException])
  def initIllegalEpochTest(): Unit = {
    new TierTopicInitLeader("my-topic", 0, -1, UUID.randomUUID, 33)
  }
