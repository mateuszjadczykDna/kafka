...
//            {StreamsConfig.AT_LEAST_ONCE},
    ...
    public void createTopics() throws Exception {
        ...
        CLUSTER.createTopic("irrelevant", 3, 1);
    }
    ...
    public void testStandbyTaskFailOver() throws Exception {
        ...
        streamInstanceOne = getStreamInstance(1, waitPoisonRecordReplication, numThreads);
        streamInstanceTwo = getStreamInstance(2, null, numThreads);
        streamInstanceThree = getStreamInstance(3, null, numThreads);

        CountDownLatch duplicateFound = new CountDownLatch(1);
        CountDownLatch threadDeaths = new CountDownLatch(numThreads * 3);
        streamInstanceOne.setUncaughtExceptionHandler(
            exceptionHandler(duplicateFound, threadDeaths, "one"));
        streamInstanceTwo.setUncaughtExceptionHandler(
            exceptionHandler(duplicateFound, threadDeaths, "two"));
        streamInstanceThree.setUncaughtExceptionHandler(
            exceptionHandler(duplicateFound, threadDeaths, "three"));
        ...
        log.info("Stream instance three started up, producing the poison record");
        ...
        if (eosConfig.equals(StreamsConfig.AT_LEAST_ONCE)) {
            waitForCondition(() -> {
                ReadOnlyWindowStore<String, String> instanceTwoWindowStore =
                    streamInstanceTwo.store(STORE_NAME, queryableStoreType);
        ...
        threadDeaths.await(40, TimeUnit.SECONDS);
        assertEquals(1, duplicateFound.getCount());
    }

    private UncaughtExceptionHandler exceptionHandler(CountDownLatch duplicateFound,
        CountDownLatch threadDeaths, String instanceName) {
        return (t, e) -> {
            if (e.getMessage().startsWith("Caught a duplicate key") && eosConfig.equals(StreamsConfig.EXACTLY_ONCE)) {
                duplicateFound.countDown();
            }
            threadDeaths.countDown();
            log.info("Instance {} ex handler, threadDeaths: {}", instanceName, threadDeaths.getCount(), e);
        };
    }
        ...
        builder.stream(inputTopic,
            ...
            () -> new Transformer<String, String, KeyValue<String, String>>() {
                ...
                public KeyValue<String, String> transform(String key, String value) {
                    ...
                    log.info("instance [{}] task [{}] [{}] Checking for duplicate (key={}, value={})", instanceId, context.taskId().toString(), Thread.currentThread().getName(), key, value);
                    ...
                    log.info("instance [{}] task [{}] [{}] Not a duplicate (key={}, value={})", instanceId, context.taskId().toString(), Thread.currentThread().getName(), key, value);
